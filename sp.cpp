	...
	unsigned char mk[16], unsigned char sk[16])
{
	...
	if ( verbose ) {
		...
		puts("=== Hardcoded quote retrieved from SGX enclave ===");

		eputs(hardcodedBase64QuoteFromEnclave);

		puts("=== Hardcoded quote retrieved from ISV/SP ===");

		eputs(hardcodedBase64QuoteFromIsvSp);

		puts("=== Current quote from SGX enclave ===");
	...
	// * b64quote
	if ( get_attestation_report(ias, config->apiver, hardcodedBase64QuoteFromIsvSp,
